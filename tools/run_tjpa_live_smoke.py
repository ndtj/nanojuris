"""Run a bounded live smoke for the public TJPA jurisprudence BFF."""
...
DEFAULT_OUTPUT = ROOT / "docs" / "provider-discovery" / "tjpa-bff-live-20260905-cycle60.json"
...
def _page_summary(page: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "has_full_text": all(bool(result.full_text) for result in page.results),
        "extraction_status": getattr(page.extraction_status, "value", page.extraction_status),
    }


def run(output: Path) -> dict[str, Any]:
    provider = TjpaJurisprudenciaBffProvider(NanoJurisConfig(timeout=20, rate_limit_interval=0))
    query = JurisprudenceQuery(text="responsabilidade civil", page_size=2)
    observations: dict[str, Any] = {}
    try:
        catalog = provider.get_catalog()
        observations["catalog"] = {
            "http_status": catalog.source_trace.http_status if catalog.source_trace else None,
            "courts": len(catalog.courts),
            "species": len(catalog.species),
        }
    ...
    report = {
        "schema_version": "tjpa-bff-live-v1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "mode": "live_bounded",
        "source": "tjpa_jurisprudencia_bff",
        "source_id": "tjpa_jurisprudencia_bff",
        "surface": "search",
        ...
        "summary": {
            ...
            "full_text_observed": page_1.get("has_full_text") is True,
