"""Run a bounded live smoke for the public TJSP eproc jurisprudence portal."""
...
DEFAULT_OUTPUT = ROOT / "docs" / "provider-discovery" / "tjsp-eproc-live-20260905-cycle63.json"
...
def _page_summary(page: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "source_origins": sorted(
            {
                str(result.raw.get("source_origin"))
                for result in page.results
                if result.raw.get("source_origin")
            }
        ),
    }
...
def run(output: Path) -> dict[str, Any]:
    provider = TjspEprocJurisprudenciaProvider(NanoJurisConfig(timeout=20, rate_limit_interval=0))
    ...
    try:
        ...
        if first.results:
            try:
                ...
                observations["detail"] = {
                    "status": "valid",
                    "http_status": document.source_trace.http_status
            ...
            except Exception as exc:  # noqa: BLE001 - classify access-control detail
                observations["detail"] = {
                    "status": "error",
    ...
    report = {
        "schema_version": "tjsp-eproc-live-v1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "mode": "live_bounded",
        "source": "tjsp_eproc_jurisprudencia",
        "source_id": "tjsp_eproc_jurisprudencia",
        "surface": "search",
        ...
        "summary": {
            ...
            "detail_access_controlled": detail.get("error_type") == "AccessControlRequiredError",
