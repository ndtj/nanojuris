"""Run a bounded live smoke for the public TJSC eproc jurisprudence surface.

Only transport metadata, stable identifiers and document hashes are written;
response bodies, cookies and session state are never persisted.
"""
...
DEFAULT_OUTPUT = ROOT / "docs" / "provider-discovery" / "tjsc-eproc-live-20260905-cycle54.json"
...
def run(output: Path) -> dict[str, Any]:
    provider = TjscEprocJurisprudenciaProvider(NanoJurisConfig(timeout=20, rate_limit_interval=0))
    ...
    report = {
        "schema_version": "tjsc-eproc-live-v1",
