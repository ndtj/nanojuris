"""Run a bounded live smoke for CNJ curated jurisprudence informativos."""
...
DEFAULT_OUTPUT = ROOT / "docs" / "provider-discovery" / "cnj-live-20260905-cycle64.json"
...
def _page_summary(page: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "pdf_links": sum(bool(result.raw.get("document_url")) for result in page.results),
    }


def run(output: Path) -> dict[str, Any]:
    provider = CnjJurisprudenciaProvider(NanoJurisConfig(timeout=20, rate_limit_interval=0))
    query = JurisprudenceQuery(text="cartorios", page_size=2)
    ...
    report = {
        "schema_version": "cnj-live-v1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "mode": "live_bounded",
        "source": "cnj_jurisprudencia",
        "source_id": "cnj_jurisprudencia",
        "surface": "curated_informativos",
