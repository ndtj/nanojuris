      ...
      <div class="shell">
        ...
        <section class="search-panel">
          ...
          <details class="advanced-filters">
            ...
            <div class="filters">
              <div class="field"><label for="exact_phrase">Frase exata</label><input id="exact_phrase" value="${escapeAttribute(state.filters.exact_phrase)}" /></div>
              <div class="field"><label for="all_words">Todas as palavras</label><input id="all_words" value="${escapeAttribute(state.filters.all_words)}" /></div>
              <div class="field"><label for="rapporteur">Relator</label><input id="rapporteur" value="${escapeAttribute(state.filters.rapporteur)}" /></div>
            </div>
          </details>
...
function renderDiagnostics() {
  const entries = Object.entries(state.status).filter(([, item]) =>
    ["failed", "skipped", "partial", "unknown"].includes(item.status),
  );
  if (!entries.length) return "";
  return `
    <details class="diagnostics" open>
      <summary>Diagnostico das fontes (${entries.length} observacoes)</summary>
      <div class="diagnostic-list">
        ${entries
          .map(
            ([source, item]) => {
              const sourceInfo = state.sources.find((candidate) => candidate.source === source);
              const humanSource = sourceInfo ? sourceLabel(sourceInfo) : source;
              return `
              <div class="diagnostic ${escapeHtml(item.status)}">
                <strong>${escapeHtml(humanSource)} - ${escapeHtml(statusLabel(item.status))}</strong>
...
function renderResults() {
  ...
  const pagination = `<nav class="pagination" aria-label="Paginacao"><span>pagina ${page}</span>${page > 1 ? `<button class="ghost" data-page="${page - 1}" type="button">anterior</button>` : ""}${meta.has_more ? `<button class="ghost" data-page="${page + 1}" type="button">proxima</button>` : ""}</nav>`;
  return `
...
function bindEvents() {
  ...
  document.querySelector("#exact_phrase")?.addEventListener("input", updateStateFromInputs);
  document.querySelector("#all_words")?.addEventListener("input", updateStateFromInputs);
  document.querySelector("#rapporteur")?.addEventListener("input", updateStateFromInputs);
  ...
  document.querySelectorAll("[data-page]").forEach((item) => {
    item.addEventListener("click", () => {
      state.page = Number(item.dataset.page || 1);
...
async function submitSearch(event) {
  ...
  if (event.target?.id === "search-form") state.page = 1;
  ...
  const filters = {
    ...
    exact_phrase: state.filters.exact_phrase,
    all_words: state.filters.all_words,
    rapporteur: state.filters.rapporteur,
  };
  state.loading = true;
  state.error = "";
  render();
...
function updateStateFromInputs() {
  ...
  state.filters = {
    ...
    exact_phrase: document.querySelector("#exact_phrase")?.value || "",
    all_words: document.querySelector("#all_words")?.value || "",
    rapporteur: document.querySelector("#rapporteur")?.value || "",
  };
}
