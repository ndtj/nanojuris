...
def build_sdd_artifacts(run: DiscoveryRun) -> dict[str, str]:
    metrics = run.metrics()
    statuses = (
        ", ".join(f"`{key}`={value}" for key, value in metrics["statuses"].items()) or "nenhum"
    )
    routes = sorted(
        {candidate.url for evidence in run.evidences for candidate in evidence.route_candidates}
    )
    selectors = [
        candidate for evidence in run.evidences for candidate in evidence.selector_candidates
    ]
    route_lines = (
...
def write_sdd_artifacts(
    run: DiscoveryRun, directory: str | Path, *, include_body: bool = True
) -> Path:
    target = Path(directory)
    target.mkdir(parents=True, exist_ok=True)
    (target / "evidence.json").write_text(
        json.dumps(run.to_dict(include_body=include_body), ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
