...
class TjalCjsgProvider(JurisprudenceProvider):
    ...
    def __init__(
        ...
        host = urlparse(self.config.tjal_cjsg_url).hostname or ""
    ...
    def _request_text(self, method: str, path: str, **kwargs: Any) -> str:
        ...
        if response.status is not TransportStatus.COMPLETE:
            raise SourceUnavailableError(
                f"TJAL/CJSG transport failed: {response.error_type or response.status.value}"
            )
        status_code = response.status_code
        if status_code is None:
            raise SourceUnavailableError("TJAL/CJSG transport returned no HTTP status")
        if status_code == 429:
            raise RateLimitDetectedError("TJAL/CJSG returned HTTP 429")
        if status_code >= 500:
            raise SourceUnavailableError(f"TJAL/CJSG returned HTTP {status_code}")
        if status_code >= 400:
            raise SourceUnavailableError(f"TJAL/CJSG rejected request with HTTP {status_code}")
