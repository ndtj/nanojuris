...
class TjamCjsgProvider(JurisprudenceProvider):
    ...
    def __init__(
        ...
        host = urlparse(self.config.tjam_cjsg_url).hostname or ""
    ...
    def _request_text(self, method: str, path: str, **kwargs: Any) -> str:
        ...
        if response.status is not TransportStatus.COMPLETE:
            raise SourceUnavailableError(
                f"TJAM/CJSG transport failed: {response.error_type or response.status.value}"
            )
        status_code = response.status_code
        if status_code is None:
            raise SourceUnavailableError("TJAM/CJSG transport returned no HTTP status")
        if status_code == 429:
            raise RateLimitDetectedError("TJAM/CJSG returned HTTP 429")
        if status_code >= 500:
            raise SourceUnavailableError(f"TJAM/CJSG returned HTTP {status_code}")
        if status_code >= 400:
            raise SourceUnavailableError(f"TJAM/CJSG rejected request with HTTP {status_code}")
