...
def sync_source_integral_pair_tool(
    dataset_id: str,
    ...
    *,
    ...
    """Join one official STJ metadata JSON with its integral-text ZIP."""

    active_client = client or NanoJurisClient()
    with SQLiteStore(db_path) as store:
        result = active_client.sync_source_integral_pair(
            source=source,
            dataset_id=dataset_id,
...
def search_jurisprudence_tool(
    ...
    *,
    ...
    if source.strip().lower() in {"all", "*", "unified"}:
        return _to_jsonable(
            active_client.search_many(
                ...
                oab=oab,
                precatory_number=precatory_number,
    ...
    if canonical:
        ...
        payload = search_page.to_dict()
        payload["canonical"] = True
        payload["results"] = [
            _to_jsonable(record) for record in search_page_to_canonical(search_page)
        ]
    ...
    *,
    ...
    with SQLiteStore(db_path) as store:
        ...
        records = store.query_records(
            ...
            case_class=case_class or None,
            judging_body=judging_body or None,
            ...
            judgment_date_from=judgment_date_from or None,
            judgment_date_to=judgment_date_to or None,
